                                                                ...
                                                                , uint16_t GPIO_pin)
{
    ...
    for (size_t i = 0; i < n_subscriptions; ++i)
    {
        ...
        if ((pSubscription->GPIO_port == GPIO_port)
                && (pSubscription->GPIO_pin == GPIO_pin))
        {
                           ...
                           , void * ctx)
{
    ...
    if (nullptr != subscription)
    {
        ...
        *subscription->GPIO_pin = GPIO_pin;
...
void CSubscribeBase::unsubscribe(GPIO_TypeDef * GPIO_port, uint16_t GPIO_pin)
{
    ...
    for (size_t i = 0; i < n_subscriptions; ++i)
    {
        ...
        else
        {
            ...
            if (pSubscription->GPIO_pin == GPIO_pin)
            {
                is_pin_in_use = true;
            }
        }
...
void CSubscribeEXTI::configureGPIO(subscription_t * pSubscription)
{
    ...
    HAL_NVIC_EnableIRQ(getIRQNumber(*subscription->GPIO_InitStruct.Pin));
}
 ...
 * \return      None
 */
void CSubscribeEXTI::unconfigureGPIO(subscription_t * subscription)
{
    HAL_NVIC_DisableIRQ(getIRQNumber(subscription->GPIO_pin));
}

/**\brief       Returns the IRQ number associated with a certain pin. Note that
 *              all GPIOs with the same pin number map to the same IRQn, no
 *              matter which port they belong to.
 *
 * \param       pin     - pin ID
 *

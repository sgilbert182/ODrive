...
public:
    ...
    struct Config_t {
        ...
        CSDControl::SDPins_t SDPins = {0, 0};
...
public:
    ...
    CSDControl stepDirectionControl;
...
//    uint16_t dir_pin_;
    ...
    State_t m_currentState = AXIS_STATE_UNDEFINED;
    State_t m_taskChain[STATEBUFFSIZE];
    CCBBuffer<State_t> m_taskChainCBBuffer;
...
inline auto Axis::make_protocol_definitions() {
    return make_protocol_member_list(
        make_protocol_property("error", &error_),
        make_protocol_ro_property("step_dir_active", &stepDirectionControl.step_dir_active_),
        make_protocol_ro_property("current_state", &m_currentState),
...
inline bool Axis::check_for_errors() {
...
template<typename T>
void Axis::run_control_loop(const T& update_handler) {
    while (requested_state_ == AXIS_STATE_UNDEFINED) {
        ...
        if (!checks_ok || !updates_ok || !watchdog_ok) {
            ...
            if (m_currentState != AXIS_STATE_IDLE)
                break;
        }
